...
// 手动触发运行
router.post('/runs/manual-trigger', async (req, res) => {
    try {
        const { pipelineId } = req.body;
        if (!pipelineId) {
            return res.status(400).json({ success: false, message: '请选择生产线' });
        }
        ...
        res.json({ success: true, data: newRun[0], message: '手动触发成功' });
    } catch (error) {
        logger.error('Manual trigger run error:', { message: error.message });
        res.status(500).json({ success: false, message: '手动触发运行失败' });
    }
});

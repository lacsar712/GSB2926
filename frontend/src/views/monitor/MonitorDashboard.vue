    ...
    <div class="table-card fade-in-up">
      <div class="table-header">
        ...
        <div class="header-actions">
          <el-select v-model="triggerPipelineId" placeholder="选择生产线" size="small" style="width: 180px; margin-right: 8px;">
            <el-option v-for="p in pipelineStats" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
          <el-button type="primary" size="small" :disabled="!triggerPipelineId" :loading="triggerLoading" @click="handleTriggerRun">
...
const handleTriggerRun = async () => {
  if (!triggerPipelineId.value) {
    ElMessage.warning('请先选择生产线')
    return
  }
  triggerLoading.value = true
  try {
    await api.post('/monitor/runs/trigger', { pipelineId: triggerPipelineId.value })
